...
process countSummary {
  ...
  echo "## Number of reads mapped and barcoded: \$n_mapped_barcoded" > ${meta.id}_allDup.log
  echo "## Number of pcr duplicates: \$n_pcr_duplicates" >> ${meta.id}_allDup.log
  echo "## Number of R1 mapped but R2 unmapped: \$n_R1_mapped_R2_unmapped" >> ${meta.id}_allDup.log
  ...
  then
    ...
    echo "## Number of rt duplicates: \$n_rt_duplicates" >> ${meta.id}_allDup.log
    echo "## Number of reads after PCR and RT removal (not R1 unmapped R2): \$n_unique_except_R1_unmapped_R2" >> ${meta.id}_allDup.log
  fi
  """
}